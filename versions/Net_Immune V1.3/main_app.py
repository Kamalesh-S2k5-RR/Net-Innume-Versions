                            ...
                            if self.stable_title_timer == 4 and active_title != last_window_title:
                                ...
                                if active_title in self.web_cache:
                                    ...
                                    if "[ERROR]" not in res.upper() and "[SAFE]" not in res.upper():
                                        self.root.after(0, self.trigger_alert_emotion)
                                        send_alert("⚠️ Blocked Site Accessed!", "You returned to a flagged website.")
                                        self.log_to_dashboard(f"> [Web Cache] Blocked access to known threat: {active_title[:30]}...")
                                    else:
...
if __name__ == "__main__":
    # TASKBAR AND NOTIFICATION FIX: We just set this ID to the readable app name!
